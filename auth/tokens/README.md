...
* id_tokens are used to simply identify the caller (B) between servers.  The ID token contains digitally signed information validating the caller.
...
```
I'm using [Application Default Credentials](https://developers.google.com/identity/protocols/application-default-credentials) and one way to initialize credentials is to export the environment variable __GOOGLE_APPLICATION_CREDENTIALS__ with a path
to the JSON cert file.

## Create access_token

After we assigned the role, download the certificate for A if you with to impersonate as a service account.  You may also impersonate B as a user and to do that, you will
need to initialize a client as an end user.  For more information on  that, see [Google Cloud Authentication Samples](https://github.com/salrashid123/gcpsamples).
...
Other languages have similar binding.  See 
...
Though id_tokens and access_tokens have similar acqusition flows, they are used for very different things:  as the name implies, access_tokens are used to grant access
to google resources (eg. GCS, PubSub, etc) while id_tokens simply assert the caller.
